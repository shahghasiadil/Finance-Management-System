        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    ...
                    <h3 class="page-title">{{ 'Budget Category Codes' | translateIn(this.locale)}}</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="index.html">{{ 'dashboard' | translateIn(this.locale) }}</a></li>
                        <li class="breadcrumb-item active">{{ 'budget category codes' | translateIn(this.locale) }}</li>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive">
                    <table class="table table-striped custom-table" id="receipt_transactions">
                        <thead>
                            <tr>
                                ...
                                <th>{{ 'budgetcode' | translateIn(this.locale) }}</th>
                        ...
                        <tbody>
                            <tr v-for="budgetcatcode in budgetcatcodes.data" :key="budgetcatcode.id">
                                <td>{{budgetcatcode.id}}</td>
                                <td>{{budgetcatcode.name}}</td>
                                <td>{{budgetcatcode.code}}</td>
                                <td>{{budgetcatcode.description}}</td>
                                <td>{{budgetcatcode.budgetcode.code}}</td>
                                <td>{{budgetcatcode.type}}</td>
                                <td class="text-right">
                                    <div class="dropdown dropdown-action">
                                        <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item" href="#" @click="edit_modal(budgetcatcode)"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                            <a class="dropdown-item" href="#" @click="delete_budgetcatcodes(budgetcatcode.id)"><i class="fa fa-trash m-r-5"></i> Delete</a>                                                
                                        </div>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <pagination :data="budgetcatcodes" @pagination-change-page="getResults">
            ...
            <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 v-show="edit_mode" class="modal-title">Edit Budget Code</h5>
                        <h5 v-show="!edit_mode" class="modal-title">Add Budget Code</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form @submit.prevent="edit_mode ? update_budgetcatcode() : create_budgetcatcode()" >
                            <div class="row">
                                <div class="col-sm-12">
                                ...
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" v-model="form.budget_code_id" name="budgetcode" title="budgetcode" data-live-search="false" >
    ...
    export default {
        ...
        methods: {
            ...
            search_it() {
                ...
                axios.get('search_budgetcatcode?q=' + query)
                .then((data) => {
                    this.budgetcatcodes = data.data
                })
            ...
            getResults(page = 1) {
                ...
                axios.get('get_budgetcatcode/?page=' + page)
                    .then(response => {
                        this.$Progress.start();

                        this.budgetcatcodes = response.data;
            ...
            edit_modal(budgetcatcode) {
                this.edit_mode = true;
                this.form.reset();
                $('#add_project').modal('show');
                this.form.fill(budgetcatcode);
            },

            //this method goes to customer.store and store customer data
            create_budgetcatcode() {
                 this.form.post('budgetcatcode')
                .then((res)=>{
                    ...
                    if(this.locale == "en")
                    {
                        toast.fire({
                        icon: 'success',
                        title: 'Budget Category Code created successfully'
                        });
                    }
            ...
            // update project
            update_budgetcatcode() 
            {
                this.form.put('budgetcatcode/'+this.form.id)
                .then((res)=>{
                    ...
                    if(this.locale == "en")
                    {
                        toast.fire({
                        icon: 'success',
                        title: 'Budget Category Code updated successfully'
                        });
                    }
                    else if(this.locale == "fa")
                    {
            ...
            load_budgetcodes() {
                axios.get('get_budgetcodes').then(({data}) => (this.budgetcodes = data));
              
            },
            load_budgetcatcodes() {
                axios.get('get_budgetcatcode').then(({data}) => (this.budgetcatcodes = data));
              
            },

            delete_budgetcatcodes(id) {
                if(this.locale == "en")
                {
                    swal.fire({
                    title: 'Are you sure?',
                    ...
                    confirmButtonText: 'Yes, delete it!'
                        ...
                        if (result.isConfirmed) {
                            ...
                            this.form.delete('budgetcatcode/'+id).then(()=>{
                                swal.fire(
                                'Deleted!',
